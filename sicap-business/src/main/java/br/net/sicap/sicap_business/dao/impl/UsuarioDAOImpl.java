...
public class UsuarioDAOImpl extends JdbcTemplate implements IUsuarioDAO {
	...
	private static final String UPDATE = "UPDATE UCTabUsers SET UCUserName =?,UCPassword =?, UCLogin= ?, UCPASSEXPIRED =?,"
	  + "UCEmail=?, UCPrivileged=?,UCInative=? where UCIdUser = ?";
	...
	public UsuarioVO listUsersById(int uCIdUser) {
		// TODO Auto-generated method stub
		return this.queryForObject(LISTBYID, new Object[] { uCIdUser }, new int[] { Types.INTEGER },
				new BeanPropertyRowMapper<UsuarioVO>(UsuarioVO.class));
	}

	public boolean update(UsuarioVO vO) {
		// TODO Auto-generated method stub
		this.update(UPDATE, new Object[] { vO.getUCUserName(), vO.getUCPassword(), vO.getUCLogin(),
				vO.getUCPASSEXPIRED(), vO.getUCEmail(), vO.getUCPrivileged(), vO.getUCInative(), vO.getUCIdUser()});
		return true;
	}
}
